...
class NewUserNotifier {
	...
	public function execute() {
		...
		$myFile = "logs/outputPHPFile.txt";
$fh = fopen($myFile, 'a') or die("cannot open file");
$stringDate = time();
fwrite($fh, "[" . date("D F d y H:i:s", $stringDate) . "]");
$stringData = " ----- From IA New User Hook Extension --> A new user has been created"
fwrite($fh, $stringData);
$user=" some name";
print(date("D F d y H:i:s", $stringDate));
fwrite($fh, $user . "\n");
fclose($fh);
